...
const CACHE_FILE_PATH = path.join(process.cwd(), 'data', 'bootstrap-static.json');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!forceRefresh && fs.existsSync(CACHE_FILE_PATH)) {
      console.log('[Backend] Returning cached bootstrap-static data');
    ...
    console.log(`[Backend] Token preview: ${cleanToken.substring(0, 50)}...`);
